     ...
     @delete-schedule.window="
         Swal.fire({
             ...
             if (result.isConfirmed) {
                 fetch(`/admin/schedules/${$event.detail}`, {
                     method: 'DELETE',
...
>
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200">
        <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            <h2 class="text-sm font-semibold text-slate-700">Daftar Ibadah Umum</h2>
        ...
        <div id="schedule-table-container">
            @include('admin.schedule.partials.table')
        </div>
    </div>
            ...
            <div x-show="modal === 'create'" x-transition class="inline-block w-full max-w-lg p-0 my-8 overflow-hidden text-left align-bottom transition-all transform bg-white rounded-2xl shadow-xl sm:align-middle">
                <div class="p-6">
                    <div class="flex items-center justify-between mb-5">
                        <h3 class="text-lg font-bold text-slate-800">Tambah Jadwal</h3>
                    ...
                    <form id="create-schedule-form" onsubmit="submitForm(event, this)" enctype="multipart/form-data">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="text" name="title" required placeholder="Ibadah Minggu" class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
                            </div>
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-[12.5px] font-medium text-slate-600 mb-1">Sektor</label>
                                    <input type="text" name="sector" placeholder="Sektor Yerikho" class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
                                </div>
                                <div>
                                    <label class="block text-[12.5px] font-medium text-slate-600 mb-1">Lokasi</label>
                                    <input type="text" name="location" placeholder="Gedung Gereja GBKP" class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
                                </div>
                            </div>
                            <div>
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div x-show="modal === 'edit'" x-transition class="inline-block w-full max-w-lg p-0 my-8 overflow-hidden text-left align-bottom transition-all transform bg-white rounded-2xl shadow-xl sm:align-middle">
                <div class="p-6">
                    ...
                    <form id="edit-schedule-form" onsubmit="submitEditForm(event, this)" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="space-y-4">
                            <div>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-[12.5px] font-medium text-slate-600 mb-1">Sektor</label>
                                    <input type="text" name="sector" class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
                                </div>
                                <div>
                                    <label class="block text-[12.5px] font-medium text-slate-600 mb-1">Lokasi</label>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label class="block text-[12.5px] font-medium text-slate-600 mb-1">Waktu</label>
                                    <input type="text" name="time" class="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
                                </div>
                            </div>
                            <div>
                                <label class="block text-[12.5px] font-medium text-slate-600 mb-1">Deskripsi</label>
...
function submitForm(e, form) {
    ...
    fetch('/admin/schedules', { method: 'POST', body: formData, headers: { 'X-Requested-With': 'XMLHttpRequest', 'X-CSRF-TOKEN': csrf() } })
    .then(r => {
        if (r.status === 422) return r.json().then(err => { throw err; });
        return r.json();
    })
    .then(() => {
        Swal.fire({ icon: 'success', title: 'Berhasil', text: 'Jadwal berhasil ditambahkan', timer: 1500, showConfirmButton: false, customClass: { popup: 'rounded-2xl' } });
...
function submitEditForm(e, form) {
    ...
    .then(() => {
        Swal.fire({ icon: 'success', title: 'Berhasil', text: 'Jadwal berhasil diupdate', timer: 1500, showConfirmButton: false, customClass: { popup: 'rounded-2xl' } });
...
function loadTable(page = 1) {
    fetch(`/admin/schedules/umum?page=${page}&${getParams()}`, { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
    .then(r => r.text())
...
function openEdit(id) {
    fetch(`/admin/schedules/${id}/edit`, { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
    .then(r => r.json())
    .then(data => {
